        ...
        for (Row row : sheet) {
            if (r > 0) {// disregard the headers
                ...
                if (!dataFormatter.formatCellValue(row.getCell(0)).isEmpty()) {
                            ...
                            case 6 -> {
                                if (!isStringBlank(cellValue)) {
                                    try {
                                        ...
                                        confGames = 0;
                                        LOGGER.warn("No integer found for " + conferenceShortName + " confGames. Defaulting to 0");
                                    }
                                }
                            }
                            case 7 -> {
                                if (!isStringBlank(cellValue)) {
                                    try {
                                        ...
                                        startWeek = 0;
                                        LOGGER.warn("No integer found for " + conferenceShortName + " startWeek. Defaulting to 0");
                                    }
                                }
                            }
                            case 8 -> {
                    ...
                    conferences.add(new Conference(conferenceId, conferenceName, conferenceShortName, abbreviation, classification, powerConf, confGames, startWeek, logo));
        ...
        for (Row row : sheet) {
            ...
            if (r > 0) {
                ...
                if (sewn <= 15 || awaySchool != null) {// only add bowl games if games already are set!
                    if (awaySchool == null) {
                        homeSchool = new School.Builder().withTgid(gatg)
                                .withConference(blankConference).withRivals(new ArrayList<>()).build();
                        allSchools.add(awaySchool);
                    }
                    if (homeSchool == null) {
                        homeSchool = new School.Builder().withTgid(ghtg)
                                .withConference(blankConference).withRivals(new ArrayList<>()).build();
