...
public class ExcelConferenceRepository implements ConferenceRepository {
	...
	public Conference findById(int id) {
		return conferenceList.stream().filter(conference -> conference.getConferenceId() == id).findFirst()
				.orElse(null);
	}
	...
	public Conference findByShortName(String conferenceShortName) {
		return conferenceList.stream().filter(c -> c.getShortName().equals(conferenceShortName)).findFirst()
				.orElse(null);
	}
