...
//	    
    ...
    private ArrayList<Integer> findEmptyWeeksInConferenceHelper(ArrayList<Integer> s1weeks, ArrayList<Integer> s2weeks, int confGamesStartDate) {
    ...
    private ArrayList<Integer> fixNoEmptyWeeks(School s1, School s2) {
        ...
        for (Game game : s1.getSchedule()) {
            if (s2EmptyWeeks.contains(game.getWeek())) {
                ...
                ArrayList<Integer> jointEmptyWeeks = findEmptyWeeksInConferenceHelper(s1EmptyWeeks, opponentEmptyWeeks, confGamesStartDate);

                //move the game to another week
                if (!jointEmptyWeeks.isEmpty()) {
                    System.out.println("Moving game: " + game.getAwayTeam() + " at " + game.getHomeTeam() + ", because no available week was found for " + s1 + " vs " + s2);
