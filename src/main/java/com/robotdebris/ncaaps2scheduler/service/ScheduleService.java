        ...
        while (!schoolsThatNeedGames.isEmpty()) {
            ...
            while (needGames) {
                ...
                if (optionalOpponent.isPresent()) {
                    ...
                    // schedule FCS game
                    List<School> fcsSchools = schoolRepository.findByNCAADivision(NCAADivision.FCS)
                            .stream()
                            .filter(opponent -> isEligibleNonConfMatchupWithEmptyWeeks(school, opponent))
                            .toList();
...
//   }
    ...
    public int getNumOfHomeConferenceGamesForSchool(School school) {
        return (int) getScheduleBySchool(school).stream()
                .filter(Game::isConferenceGame)
